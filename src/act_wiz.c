...
/*
void do_saveconcept(CHAR_DATA *ch, char *argument)
{
	CHAR_DATA *vch;
	bool fConfirm = FALSE;
	char arg[MAX_INPUT_LENGTH]={'\0'};
	...
	if(IS_NULLSTR(argument))
	{
		send_to_char("Syntax: saveconceptfile <person> [confirm]\n\r", ch);
		return;
	}
	...
	if(!str_cmp(argument, "confirm")) fConfirm = TRUE;

	if(!fConfirm)
	{
		send_to_char("Are you sure that character should become a concept file?\n\r", ch);
		send_to_char("(The character will be deleted after being dumped.)\n\r", ch);
		send_to_char("Please use the confirm option to force the issue.\n\r", ch);
		return;
	}

	closeReserve();

	if ( ( fp = fopen( (char *)Format("%s%ld.%s", CONCEPT_DIR, (long)current_time, race_table[vch->race].name), "w" ) ) == NULL )
	{
		send_to_char( "Unable to open file.", ch);
	}
	else
	{
		send_to_char("Saving concept...\n\r", ch);
		fprintf( fp, "#%s\n", "PLAYER" );
		fprintf( fp, "Race %d\n",   ch->race                );
		...
		fprintf( fp, "RGW %d %d %d %d %d %d\n", ch->RBPG, ch->max_RBPG, ch->GHB,
				ch->max_GHB, ch->willpower, ch->max_willpower );
		fprintf( fp, "Gen %d\n", ch->gen                    );
		fprintf( fp, "Dsc " );
		...
		fprintf( fp, "Attr %d %d %d %d %d %d %d %d %d\n",
				ch->perm_stat[STAT_STR], ch->perm_stat[STAT_DEX], ch->perm_stat[STAT_STA],
				ch->perm_stat[STAT_CHA], ch->perm_stat[STAT_MAN], ch->perm_stat[STAT_APP],
				ch->perm_stat[STAT_PER], ch->perm_stat[STAT_INT], ch->perm_stat[STAT_WIT] );

		fprintf( fp, "Cncpt %s~\nNature %s~\nDemnor %s~\n", ch->concept, ch->nature, ch->demeanor );

		for ( i = 0; i < MAX_ABIL; i++ )
		{
			if ( ability_table[i].name != NULL
					&& IS_ATTRIB_AVAILABLE(ch->race,i))
			{
				fprintf( fp, "Ab %d '%s'\n", ch->ability[i].value, ability_table[i].name );
			}
		}
		...
		send_to_char("Done.\n\r", ch);
		send_to_char("Deleting character...\n\r", ch);
		vch->pcdata->confirm_delete = TRUE;
		send_to_char("This character has been converted into a concept.\n\r", vch);
		send_to_char("You will now be deleted.\n\r", vch);
		do_function(vch, &do_delete, "" );

		load_char_obj(&d, strsave, FALSE, TRUE, FALSE);
		if(d.character != NULL)
		{
			d.character->next = concept_list;
			concept_list = d.character;
			d.character = NULL;
		}
	}
	fclose( fp );
