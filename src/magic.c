...
void do_thaumaturgy1 (CHAR_DATA *ch, char *string)
{
	...
	if( ch->race != race_lookup("vampire") || ch->disc[DISC_THAUMATURGY] < 1)
	{
		send_to_char("You do not know Taste of Blood.\n\r", ch);
		return;
	}
	...
	/* Check for object to be touched. */
	if((obj = get_obj_here(ch, string)) == NULL)
	{
		send_to_char("You can't find anything like that.\n\r", ch);
		return;
	}

	/* Check object for information on type. */
	if(obj->item_type != ITEM_LIQUID)
	{
		send_to_char("That's not even a liquid.\n\r", ch);
		return;
	}

	if(obj->value[0] == 0)
	{
		send_to_char("It's not blood!\n\r", ch);
		return;
	}

	/* Check for success. Current Willpower vs. diff 4. */
	if((fail = dice_rolls(ch, ch->willpower, diff)) == 0)
	{
		send_to_char("You fail to determine anything about the blood.\n\r", ch);
		return;
	}
	...
	if(fail < 0)
	{
		...
		{
		case 0:
			send_to_char("There is a burning rage in the blood.\n\r", ch);
			break;
		case 1:
			send_to_char("The blood is potent enough to belong to an antideluvian.\n\r", ch);
			break;
		case 2:
			send_to_char("The blood belongs to a madman.\n\r", ch);
			break;
		case 3:
			send_to_char("The blood belongs to a caitiff.\n\r", ch);
			break;
		case 4:
			send_to_char("The blood belongs to a mortal.\n\r", ch);
			break;
		case 5:
			send_to_char("The blood belongs to an animal.\n\r", ch);
			break;
		case 6:
			send_to_char("It looks tasty.\n\r", ch);
			break;
		case 7:
			send_to_char("The blood belongs to Marcia.\n\r", ch);
			break;
		case 8:
			send_to_char("The blood is weak.\n\r", ch);
			break;
		case 9:
			send_to_char("The blood belongs to a werewolf.\n\r", ch);
			break;
		case 10:
			send_to_char("The blood belongs to Rosario.\n\r", ch);
			break;
		}
	}
	else
	{
		...
		{
			/* All info. */
			send_to_char("Race, Strength, Clan and Name", ch);
		}
		else if(fail > 2)
		{
			/* 3 pieces */
			send_to_char("Race, Strength or Clan", ch);
		}
		else if(fail == 2)
		{
			/* 2 pieces */
			send_to_char("Race or Strength", ch);
		}
		else
		{
			send_to_char( Format("%s blood makes him a something.", victim->name), ch);
		}
	}
